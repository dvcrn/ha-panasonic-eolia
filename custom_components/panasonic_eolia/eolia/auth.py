...
class PanasonicEolia:
    def __init__(self, username: str, password: str, session: Optional[aiohttp.ClientSession] = None):
        if session:
            ...
            raise ValueError("aiohttp session must be provided")
    ...
    async def step1_authorize(self) -> bool:
        ...
        async with self.session.get(
            ...
            if response.status != 302:
                raise Exception(f"Expected redirect, got {response.status}")
    ...
    async def step2_login_page(self) -> bool:
    ...
    async def step3_challenge(self) -> bool:
        ...
        async with self.session.post(
            ...
            if response.status != 200:
                raise Exception(f"Challenge failed with status {response.status}")

        return True

    async def step4_login(self) -> bool:
        ...
        async with self.session.post(
            ...
            if response.status != 200:
                text = await response.text()
                raise Exception(f"Login failed with status {response.status}: {text}")

            text = await response.text()

        # The response is an HTML form that needs to be submitted
        # Extract form data
        wa_match = re.search(r'name="wa"\s+value="([^"]+)"', text)
    ...
    async def step4b_callback(self, wa: str, wresult: str, wctx: str) -> bool:
        ...
        async with self.session.post(
            ...
            _LOGGER.debug(f"Callback response status: {response.status}")
    ...
    async def step4c_authorize_resume(self, resume_state: str) -> bool:
        ...
        async with self.session.get(
            ...
            _LOGGER.debug(f"Resume response status: {response.status}")
    ...
    async def step5_token_exchange(self) -> bool:
        ...
        async with self.session.post(
            ...
            if response.status != 200:
                text = await response.text()
                raise Exception(f"Token exchange failed with status {response.status}: {text}")
    ...
    async def get_devices(self) -> List[Appliance]:
        ...
        async with self.session.get(
            ...
            if response.status == 200:
                ...
                text = await response.text()
                _LOGGER.error(f"Failed to fetch devices: {response.status} - {text}")
                return []

    async def get_product_functions(self, product_code: str) -> Optional[ProductFunctionsResponse]:
        ...
        async with self.session.get(
            ...
            if response.status == 200:
                ...
                return ProductFunctionsResponse.from_dict(data)
            else:
                text = await response.text()
                _LOGGER.error(f"Failed to fetch product functions: {response.status} - {text}")
                return None
    ...
    async def get_device_status(self, device_id: str) -> Optional[DeviceStatus]:
        ...
        async with self.session.get(
            ...
            if response.status == 200:
                ...
                text = await response.text()
                _LOGGER.error(f"Failed to fetch device status: {response.status} - {text}")
                return None

    async def update_device_status(self, device_id: str, status: UpdateDeviceRequest) -> Optional[DeviceStatus]:
        ...
        async with self.session.put(
            ...
            if response.status == 200:
                ...
                text = await response.text()
                _LOGGER.error(f"Failed to update device status: {response.status} - {text}")
                return None